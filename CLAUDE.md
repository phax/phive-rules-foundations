...
**phive-rules-foundations** is a Maven multi-module project providing the *foundational* validation rules for [PHIVE](https://github.com/phax/phive) (Philip Helger Integrative Validation Engine): the pure structural (XSD only, **no Schematron**) document formats that other rule sets build upon. The shared registration SPI and helper classes used across the whole phive-rules ecosystem live in the separate `phive-rules-shared` project (sibling repo at `../phive-rules-shared`, Maven artifact `com.helger.phive.rules:phive-rules-shared`), which every module here depends on.
...
- `phive-rules` (the sibling repo) imports this project's parent POM as a BOM and depends on the artifacts built here, so **this project must be installed/released before `phive-rules` builds**. Locally: run `mvn install` here first. In turn, this project depends on `phive-rules-shared`, so **`phive-rules-shared` must be installed/released before this project builds** (run `mvn install` in `../phive-rules-shared` first).
...
Package `com.helger.phive.rules.shared`, Maven artifact `com.helger.phive.rules:phive-rules-shared` (sibling repo `../phive-rules-shared`). Provides the shared base used by all rule modules (in this repo **and** in `phive-rules`):
...
- `DVRHelper` — creates `DVRCoordinate` instances with version parsing (`createCoordinate`).
...
Dependencies of `phive-rules-shared` are deliberately minimal: `slf4j-api` + `phive-xml` (which transitively brings ph-commons, ph-diver and ph-schematron). It has **no** dependency on any document-binding library.
...
Every module ships a `{Format}ValidationSPI` implementing `com.helger.phive.rules.shared.IValidationRulesRegistrarSPI` (annotated `@IsSPIImplementation`, listed in `src/main/resources/META-INF/services/com.helger.phive.rules.shared.IValidationRulesRegistrarSPI`). Its `registerValidationRules` delegates to the module's `init…` method(s). These foundation formats have no cross-module prerequisites, so they do not override `getAllPrerequisites()`.
...
| Dependency | Purpose |
|---|---|
| `phive-rules-shared` (1.0.0) | Shared registration SPI + helpers (external, `../phive-rules-shared`) |
...
- The shared SPI/helpers are in `com.helger.phive.rules.shared` (the external `phive-rules-shared` project). Their former locations in `com.helger.phive.rules.api` still exist in `phive-rules` as `@Deprecated` delegates — do not use those.
